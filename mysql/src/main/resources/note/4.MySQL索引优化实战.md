...
* [MySQL 索引优化实战](#mysql-索引优化实战)
* [实战测试](#实战测试)
    * [索引下推](#索引下推)
    * [MySQL 是如何计算的](#mysql-是如何计算的)
* [常见 sql 深入优化](#常见-sql-深入优化)
    * [Order by 与 Group by 优化](#order-by-与-group-by-优化)
    * [Using filesort 文件排序原理详解](#using-filesort-文件排序原理详解)
        * [filesort 文件排序方式](#filesort-文件排序方式)
* [索引设计原则](#索引设计原则)
    * [设计三星索引](#设计三星索引)
    * [多列索引设计原则](#多列索引设计原则)
    * [分页查询优化(limit)](#分页查询优化limit)
    * [Join 关联表查询优化](#join-关联表查询优化)
        * [嵌套循环连接算法 Nested-Loop Join(NLJ)算法](#嵌套循环连接算法-nested-loop-joinnlj算法)
        * [基于块的嵌套循环连接 Block Nested-Loop Join(BNL)算法](#基于块的嵌套循环连接-block-nested-loop-joinbnl算法)
            * [对于小表定义的明确](#对于小表定义的明确)
            * [in和 exsits 优化](#in和-exsits-优化)
    * [count(*) 优化](#count-优化)
    * [MySQL 查询成本计算](#mysql-查询成本计算)
    * [MySQL 调优金字塔](#mysql-调优金字塔)
...
```
## MySQL 调优金字塔
![MySQL调优金字塔](../photo/14.MySQL调优金字塔.png)
MySQL 的调优不仅仅只限于 SQL 层面的调优，还有硬件层面、设计架构方面。比如从硬件方面设计阵列什么的，架构方面搭建集群的。
硬件和OS 的调优成本是最高的，但是效果是最好的。然后一次类推。
